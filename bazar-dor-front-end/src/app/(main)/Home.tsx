...
export function Home() {
  ...
  return (
    <div className="pb-12">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          ...
          <div className="bg-rose-50/80 backdrop-blur-md border border-rose-100 rounded-2xl p-4 flex items-start gap-3 animate-in slide-in-from-top-4 fade-in duration-500 cursor-pointer hover:bg-rose-100/80 transition-colors relative overflow-hidden">
            ...
            <div className="shrink-0 mt-0.5"><span className="text-xl">⚠️</span></div>
            ...
            <span className="shrink-0 text-rose-400 text-lg mt-2">→</span>
          </div>
        ...
        <div className="lg:w-80 xl:w-96 flex-shrink-0 flex flex-col gap-4">
          ...
          {/* Daily Basket Index */}
          <div
            className="glass-card p-6 flex flex-col gap-4 cursor-pointer hover:shadow-md transition-shadow"
            ...
            <div className="h-12 w-full mt-2">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={sparklineData}>
                  <Line type="monotone" dataKey="value" stroke="#10B981" strokeWidth={2.5} dot={false} isAnimationActive={true} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="glass-card p-5 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-emerald-50/60 rounded-2xl p-4 text-center">
                <p className="text-2xl font-black text-[#064E3B] font-num">১২৮</p>
                <p className="text-xs text-slate-500 mt-1">নতুন দাম</p>
              </div>
              <div className="bg-blue-50/60 rounded-2xl p-4 text-center">
                <p className="text-2xl font-black text-blue-700 font-num">৪৩</p>
                <p className="text-xs text-slate-500 mt-1">বাজার আপডেট</p>
              </div>
              <div className="bg-amber-50/60 rounded-2xl p-4 text-center">
                <p className="text-2xl font-black text-amber-700 font-num">৭</p>
                <p className="text-xs text-slate-500 mt-1">জরুরী এলার্ট</p>
              </div>
              <div className="bg-rose-50/60 rounded-2xl p-4 text-center">
                <p className="text-2xl font-black text-rose-700 font-num">৩</p>
                <p className="text-xs text-slate-500 mt-1">স্টক আউট</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── Product Details Bottom Sheet / Desktop Modal ── */}
      {selectedProduct && (
                ...
                <div className="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-slate-50 flex flex-col justify-between">
                  ...
                  <div className="flex-1 flex items-end">
                    <svg viewBox="0 0 100 40" className="w-full h-12 overflow-visible">
                      <path d="M0,30 C20,30 30,10 50,20 C70,30 80,5 100,15" fill="none" stroke="#064E3B" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm" />
      ...
      {/* ── Market Index Details Sheet / Desktop Modal ── */}
      {isIndexSheetOpen && (
        <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-300">
          ...
          <div className="w-full lg:max-w-lg h-[85vh] lg:h-auto lg:max-h-[85vh] bg-[#FAFCFC] rounded-t-[32px] lg:rounded-[32px] shadow-2xl relative z-10 flex flex-col animate-in slide-in-from-bottom-full lg:zoom-in-95 duration-300">
            ...
            <div className="hidden lg:flex justify-end px-6 pt-5 pb-1 shrink-0">
              <button
                onClick={() => setIsIndexSheetOpen(false)}
                className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-8 flex flex-col gap-4">
                ...
                <div className="h-48 w-full relative">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={indexChartData} margin={{ top: 5, right: 0, left: 0, bottom: 0 }}>
                      ...
                      <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#94A3B8', fontSize: 10, fontFamily: 'Noto Sans Bengali' }} minTickGap={30} />
                      <YAxis orientation="right" axisLine={false} tickLine={false} tick={{ fill: '#94A3B8', fontSize: 10, fontWeight: 600, fontFamily: 'Inter' }} domain={[1100, 1400]} tickFormatter={(value) => `৳${value}`} width={45} />
                      <Tooltip
                      ...
                      />
                      <Line type="monotone" dataKey="value" stroke="#064E3B" strokeWidth={3} dot={false} activeDot={{ r: 5, fill: '#064E3B', stroke: '#fff', strokeWidth: 2 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-white/60 backdrop-blur-xl border border-white rounded-3xl shadow-sm p-5 flex flex-col">
                ...
                <div className="flex flex-col">
                  {[
                    { name: 'চাল', qty: '৫ কেজি', price: '৳৩৪০' },
                    { name: 'সয়াবিন তেল', qty: '১ লিটার', price: '৳১৬০' },
                    { name: 'মুরগি', qty: '১ কেজি', price: '৳১৫০' },
                    { name: 'পেঁয়াজ', qty: '১ কেজি', price: '৳১২০' },
                    { name: 'আলু', qty: '২ কেজি', price: '৳১১০' },
                  ].map((item, i, arr) => (
                    <div key={item.name} className={`flex justify-between items-center py-3 ${i < arr.length - 1 ? 'border-b border-slate-200/60 border-dashed' : ''}`}>
                      <span className="text-slate-700 font-medium">{item.name} <span className="text-slate-400 text-sm font-normal">({item.qty})</span></span>
